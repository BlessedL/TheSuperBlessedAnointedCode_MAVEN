...
public class ProxyServerFrontendHandler extends ChannelInboundHandlerAdapter {
    ...
    private EpollSocketChannel outboundChannel;
    private EpollSocketChannel inboundChannel;
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        inboundChannel = (EpollSocketChannel) ctx.channel();
                ...
                if (connected) {
                    ...
                    if (outboundChannel != null) {
                        if (outboundChannel.isActive()) {
                            ...
                            final EpollSocketChannel theOutboundChannel = outboundChannel;
                            ...
                            outboundChannel.writeAndFlush(msg).addListener(new ChannelFutureListener() {
                                ...
                                public void operationComplete(ChannelFuture future) {
                                    if (future.isSuccess()) {
                                        ...
                                        //---ctx.channel().read();
                                        final EpollSocketChannel theInboundChannel = (EpollSocketChannel) ctx.channel();
                                        theInboundChannel.spliceTo(theOutboundChannel, Integer.MAX_VALUE);
                                    } else {
                            ...
                            if (!connectionMsgReceived) {
                                ...
                                if (!pathLengthSet) {
                                    ...
                                    if (pathBuf.readableBytes() >= pathLength) {
                                        ...
                                        restOfMsgBuf = ctx.alloc().buffer(remainingBytes);
                                    }
                                } else {
                                    if (!readInRestOfMsg) {
                                        restOfMsgBuf.writeBytes(in, ((restOfMsgBuf.writableBytes() >= in.readableBytes()) ? in.readableBytes() : restOfMsgBuf.writableBytes()));
                                        if (restOfMsgBuf.readableBytes() >= remainingBytes) {
                                            readInRestOfMsg = true;
                                            ...
                                            if (!connected) {
                                                ...
                                                b.group(inboundChannel.eventLoop())
                                                        ...
                                                        .option(EpollChannelOption.AUTO_READ, false) //Autoread need to be false, so I can read only when data is available
							                            .option(ChannelOption.SO_SNDBUF, 100 * 1024 * 1024);
                                                ChannelFuture f = b.connect(remoteHost, remotePort);
                                                outboundChannel = (EpollSocketChannel)f.channel();
                                                ...
                                                f.addListener(new ChannelFutureListener() {
                                                                  ...
                                                                  EpollSocketChannel myInboundChannel;
                                                                  EpollSocketChannel myOutboundChannel;
                                                                  ByteBuf theByteBuf;

                                                                  public ChannelFutureListener init(ProxyServerFrontendHandler aProxyServerFrontendHandler, EpollSocketChannel anInboundChannel, EpollSocketChannel anOutboundChannel) {
                                                                      //public ChannelFutureListener init(ProxyServerFrontendHandler aProxyServerFrontendHandler, Channel anInboundChannel, Channel anOutboundChannel, ByteBuf aByteBuf) {
                                                                  ...
                                                                  public void operationComplete(ChannelFuture future) {
                                                                      if (future.isSuccess()) {
                                                                          ...
                                                                          //myProxServerFrontendHandler.sendConnectionMsg(myInboundChannel, myOutboundChannel, theByteBuf);
                                                ...
                                                //}.init(this, inboundChannel, outboundChannel,restOfMsgBuf));
                                            }//End (!connected)
                                        } //End checking to see if read in the rest of the remaining bytes
                                    }//End read rest of MSG
                                }//else read in all the path bytes
                            }//end connectionMsgReceived
    ...
    //public void sendConnectionMsg(Channel anInboundChannel, Channel anOutboundChannel, ByteBuf aByteBuf){
    public void sendConnectionMsg(EpollSocketChannel anInboundChannel, EpollSocketChannel anOutboundChannel){
        try {
            //ByteBuf theRemainingByteBuf = aByteBuf;
           // if (theRemainingByteBuf != null){
           //     logger.info("ProxyServerFrontEndHandler: sendConnectionMsg: The NUMBER OF REMAINING BYTES THAT WILL BE SENT FROM THE REMAINING BYTE BUF = " + theRemainingByteBuf );
           // }
            ...
            if (anOutboundChannel != null) {
                ...
                if (newPathToSend !=null){
                    ...
                    if (restOfMsgBuf != null) {
                        logger.info("ProxyServerFrontEndHandler: SendConnectionMsg: REST OF MSG BUFFER READABLE BYTES = " + restOfMsgBuf.readableBytes()  );
                        if (restOfMsgBuf.readableBytes() >= remainingBytes) {
                            anOutboundChannel.write(restOfMsgBuf);
                        }
                    } else {
                        logger.info("ProxyServerFrontEndHandler: SendConnectionMsg: REST OF MSG BUFFER = NULL"  );
                    }
                ...
                //SPLICE THE CHANNELS - Since I'm using splice I'm not calling inboundChannel.read()
                //Splicing allows to directly transfer bytes from one filedescriptor to another without copy data from kernel to user-space
                // Integer.MAX_VALUE will splice infinitly.
                /*
                inboundChannel.spliceTo(anOutboundChannel, Integer.MAX_VALUE).addListener(new ChannelFutureListener() {
                    @Override
