...
public class FileReceiver implements Runnable {
    ...
    //myRegisteredChannels = a HashMaps of NetworkPaths where each Network Path Entry has a Hashmap of Control Channels associated with it
    //myRegisteredChannels = HashMap<PathName, HashMap<ControlChannel ID, Control Channel Object>>
    public static HashMap<String,HashMap<String,FileReceiver.ControlChannelObject>> myRegisteredChannels = new HashMap<String,HashMap<String,FileReceiver.ControlChannelObject>>();
    ...
    public static class ControlChannelObject{
        ...
        public ControlChannelObject(int aControlChannelId, List<FileReceiver.DataChannelObject> aDataChannelList, int aParallelNum ){
        ...
        public ControlChannelObject(int aControlChannelId, int aParallelNum){
            myControlChannelId = aControlChannelId;
        ...
        public ControlChannelObject(int aControlChannelId, int aDataChannelId, int aChannelType, int aParallelNum, FileReceiverHandler aControlChannelHandler, FileReceiverHandler aDataChannelHandler, long aThreadId){
            myControlChannelId = aControlChannelId;
            myControlChannelHandler = null;
            parallelDataChannelNum = aParallelNum;
            //Throughput Statistics
            ...
            if (aChannelType == CONTROL_CHANNEL_TYPE) {
                ...
                if (myDataChannelList == null) {
                    //Data Channel List for this Control Channel is EMPTY
                    myDataChannelList = new LinkedList<FileReceiver.DataChannelObject>();
                    myDataChannelList.add(new FileReceiver.DataChannelObject(aDataChannelId,aThreadId, aDataChannelHandler));
                }
                else {
    ...
    public static class DataChannelObject{
        ...
        public DataChannelObject(int aDataChannelId, long aThreadId, FileReceiverHandler aDataChannelHandler){
    ...
    // Acceptor: if connection is established, assign a FileReceiverHandler to it.
    private class Acceptor implements Runnable {
        public void run() {
            try {
                SocketChannel socketChannel = _serverSocketChannel.accept();
                if (socketChannel != null) {
                    new FileReceiverHandler(_selector, socketChannel);
                }
            }
    ...
    //                                FileReceiver.registerChannel( myAliasFilePath,        myChannelType,  myControlChannelId,     myDataChannelId,  myParallelNum,     myConcurrencyNum,          this,                                   null,                                   threadId, threadName);
    public static synchronized FileReceiverHandler registerChannel(String aPathAliasName, int aChannelType, int aControlChannelId, int aDataChannelId, int aParallelNum, int aConcurrencyNum, FileReceiverHandler aControlChannelHandler, FileReceiverHandler aDataChannelHandler, long aThreadId  ){
        try {
            ...
            */
            FileReceiverHandler returnChannelHandler = null;
            ...
            if ( aPathAliasName != null) {
                ...
                if (!myRegisteredChannels.containsKey(aPathAliasName)) {
                    //ControlChannelId ControlChannelObject
                    myRegisteredChannels.put(aPathAliasName, new HashMap<String, FileReceiver.ControlChannelObject>());
                }
                ...
                HashMap<String, FileReceiver.ControlChannelObject> myControlChannelObjectMap = myRegisteredChannels.get(aPathAliasName);
                //return myHashMap2;
                ...
                if (!myControlChannelObjectMap.containsKey( String.valueOf(aControlChannelId) ) ) {
                    ...
                    //If the ControlObject Doesn't exist - Create the ChannelControlObject either with a ControlChannelHandler if a control channel is registering or with a DataChannelHandler if a DataChannel is registering
                    if (aChannelType == CONTROL_CHANNEL_TYPE) {
                        myControlChannelObjectMap.put(String.valueOf(aControlChannelId), new FileReceiver.ControlChannelObject(aControlChannelId, aDataChannelId, aChannelType, aParallelNum, aControlChannelHandler, null, aThreadId));
                    }
                    else {
                        //THIS IS A DATA CHANNEL REGISTERING SET THE CONTROL CHANNEL HANDLER TO NULL
                        myControlChannelObjectMap.put(String.valueOf(aControlChannelId), new FileReceiver.ControlChannelObject(aControlChannelId, aDataChannelId, aChannelType,  aParallelNum, null, aDataChannelHandler, aThreadId));
                    }
                    ...
                    */
                    ...
                    if (myControlChannelObject.getDataChannelObjectList_ForControlObject().size() >= myControlChannelObject.getParallelDataChannelNum_ForControlObject()){
                        ...
                        if (myControlChannelObject.getControlChannelHandler_ForControlObject() != null) {
                            returnChannelHandler = myControlChannelObject.getControlChannelHandler_ForControlObject();
                        }
                    }
                    ...
                    if (aChannelType == CONTROL_CHANNEL_TYPE) {
                        ...
                        myControlChannelObject.addDataChannelObject_ToControlObject(new FileReceiver.DataChannelObject(aDataChannelId,aThreadId,aDataChannelHandler));
    ...
    public static synchronized String registeredChannelsToString(){
        try {
            ...
            Iterator<Map.Entry<String,HashMap<String,FileReceiver.ControlChannelObject>>> pathIterator = myRegisteredChannels.entrySet().iterator();
        ...
        try {
            ...
            while (true) {
                ...
                while (it.hasNext()) {
                    ...
                    it.remove();
                    Runnable r = (Runnable) sk.attachment(); // FileReceiverHandler or acceptor callback/runnable
                    if (r != null) {
                        r.run();
                    }
                }
            ...
            System.err.printf("FileReceiver: Run error: %s %n", e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        try {
            new Thread(new FileReceiver(4959)).start(); // a single thread blocking on selector for events
        } catch (Exception e) {
            System.err.printf("FileReceiver: main error: %s %n", e.getMessage());
            e.printStackTrace();
        }
